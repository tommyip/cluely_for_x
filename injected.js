/* Runs in the page context to wrap XMLHttpRequest */
...
function userDefinedPostFilter(args) {
  // console.log(args);
  const {
    post_text,
    is_following, // I follow this account
    is_follower,  // This account follows me
    is_verified,
    followers_count,
    friends_count,
    views,
...
function timelineFilter(timelineEntry) {
  ...
  if (instructions) {
    for (const instruction of instructions) {
      if (instruction.type === "TimelineAddEntries") {
        const beforeCount = instruction.entries.length;
        const filtered = instruction.entries.filter(timelineFilter);
        console.log(`Filtered ${beforeCount - filtered.entries.length} posts`);
        instruction.entries = filtered;
      }
    }
  }
  return data;
...
(() => {
  xhook.after(function (request, response) {
    const url = new URL(request.url || response.finalUrl);
    const isHomeTimeline = url.host === "x.com" && /^\/i\/api\/graphql\/.*?\/HomeTimeline$/.test(url.pathname);
    if (!isHomeTimeline) return;
    const data = JSON.parse(response.data);
    const processed = processTimelineResponse(data);
    response.data = response.text = JSON.stringify(processed);
  });
})();
